...
from CTFd.models import db, Challenges, Files, Solves, WrongKeys, Tags, Teams, Awards, Feedbacks
...
def chals():
    ...
    if user_can_view_challenges() and (ctf_started() or is_admin()):
        ...
        for x in chals:
            ...
            json['game'].append({'id': x[1], 'name': x[2], 'value': x[3], 'description': x[4], 'category': x[5], 'files': files, 'tags': tags, 'feedbacks': feedback})
